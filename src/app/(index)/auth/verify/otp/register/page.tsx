            ...
            <div className='flex w-full justify-end items-center gap-2'>
                ...
                }
                {
                    mutationIsLoading ? <span className="text-sm font-semibold text-gray-700 hover:text-gray-500 focus:text-gray-500 hover:underline cursor-pointer">
                        ...
                        <span className="text-sm font-semibold text-gray-700 hover:text-gray-500 focus:text-gray-500 hover:underline cursor-pointer" onClick={() => allowResend && handleResendRegisterOTP()}>
                            Resend OTP
                        </span>
                }
            </div>
...
const registerUser = async (uid: string, token: string) => {
    ...
    await setCookie('access_token', response.data.access, response.data.access ? (decodeJwtToken(response.data.access)?.exp ?? 0) - Math.floor(Date.now() / 1000) : 0);
    await setCookie('refresh_token', response.data.refresh, response.data.refresh ? (decodeJwtToken(response.data.refresh)?.exp ?? 0) - Math.floor(Date.now() / 1000) : 0);
